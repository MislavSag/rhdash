...
```{r, eval=F}
# dodaj doubling time za slučajeve
sr <- C19 %>% 
  dplyr::select(time, cases, cases_cum) %>%
  dplyr::arrange(., time) 

log(2)/coef(lm(log(cases_cum)~time, sr))[[2]]
...
c_gr <- data.frame(vrijeme = sr$time[-1], stopaRasta = gr)
#ggplot(c_gr, aes(x=vrijeme, y=stopaRasta)) + geom_line()
dTime <- mutate(c_gr, doubleTime = log(2)/stopaRasta)
#ggplot(dTime, aes(x=vrijeme, y=doubleTime)) + geom_bar(stat="identity")
