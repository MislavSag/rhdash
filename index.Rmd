...
C19_plot_case <- plot_ly(data = C19, x = ~time, y = ~cases_cum) %>% 
  add_lines(., hoverinfo="text", text=paste(C19$time, ': ', C19$cases_cum)) %>% 
...
C19_plot_death <- plot_ly(data = C19, x = ~time, y = ~deaths) %>% 
  add_bars(., hoverinfo="text", text=paste(C19$time, ': ', C19$deaths)) %>% 
  layout(xaxis = list(title = ''),
