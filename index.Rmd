...
sr <- C19 %>% 
  dplyr::select(time, cases, cases_cum) %>%
  dplyr::arrange(., time) 
# dplyr::mutate(change = (cases - dplyr::lag(cases,1))/lag(cases,1)*100) %>%
#  mutate(second = dplyr::nth(cases,2),
#        second_last = dplyr::nth(cases, n = as.numeric(n()-1)))  

log(2)/coef(lm(log(cases_cum)~time, sr))[[2]]
  
  
stopaRasta <- c(NA, log(sr$second/sr$second_last)/nrow(C19))
